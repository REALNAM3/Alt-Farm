...
subprocess.Popen(["python", "Account_Detector.py"])
...
async def modson(interaction: discord.Interaction):
    ...
    all_user_ids = list({int(uid) for ids in list(ALL_MODS.values()) + list(UNKNOWN_MODS.values()) for uid in ids})
    valid_user_ids = [uid for uid in all_user_ids if uid > 0]
