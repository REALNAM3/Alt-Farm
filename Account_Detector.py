...
async def snipe(interaction: discord.Interaction, username: str):
    ...
    async with aiohttp.ClientSession(cookies={".ROBLOSECURITY": ROBLOSECURITY}) as session:
        ...
        try:
            ...
            await interaction.followup.send(f"Error fetching presence: {e}")
            return

        if "userPresences" not in presence or len(presence["userPresences"]) == 0:
            await interaction.followup.send(f"No presence data for {username}")
            return
        ...
        avatar_url = None
        try:
            async with session.get(f"https://thumbnails.roblox.com/v1/users/avatar?userIds={user_id}&size=720x720&format=Png&isCircular=true") as avatar_resp:
